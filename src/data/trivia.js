export const trivia = {
  programare_procedurala: [
    ...
    },
    {
      questionNumber: "16.",
      question:
        'Considerăm următoarea secvenţă de instrucțiuni:\n\nnp=0;\nfor(i=0;i<n;i++)\n  if(…) np++;\nprintf("%d",np);\n\nCu ce expresie dintre cele de mai jos trebuie înlocuite spaţiile punctate din secvenţă de instrucțiuni dată astfel încât aceasta să afişeze câte valori strict pozitive şi pare sunt în tabloul 𝑎, format din 𝑛 numere întregi?',
      answers: [
        "(a[i]>0)&&(a[i]%2!=0)",
        "(a[i]>0)&&(a[i]%2==0)",
        "(a[i]>0)||(a[i]%2!=0)",
    ...
    },
    {
      questionNumber: "26.",
      question:
        "Considerăm tipul de date Punct, capabil să memoreze abscisa şi ordonata unui punct din plan,\nşi tipul de date Segment, capabil să memoreze două puncte reprezentând extremităţile unui segment din plan, definite astfel:\n\ntypedef struct\n{\n  float x,y;\n}Punct;\n\ntypedef struct\n{\n  Punct A,B;\n}Segment;\n\nCare dintre următoarele expresii are o valoare nenulă dacă şi numai dacă variabila 𝑠 de tip Segment memorează informații despre un segment vertical (aflat pe axa Oy sau paralel cu axa Oy)?",
    ...
    },
    {
      questionNumber: "29.",
      question:
        "Considerăm funcția int suma(int x,int y) care returnează suma numerelor întregi x și y, precum și funcția int prod(int x,int y) care returnează produsul numerelor întregi x și y. Știind că a, b și c sunt 3 variabile de tip întreg, care dintre expresiile de mai jos atribuie\nvariabilei t de tip întreg valoarea expresiei a*b+a*b*c?",
      answers: [
        "t = suma(prod(a,",
        "t = suma(prod(a,",
        "t = suma(prod(a,",
    ...
    },
    {
      questionNumber: "35.",
      question:
        "Fie 𝑣 un tablou unidimensional format din 100 de numere reale de tip double și 𝑝 o variabilă de tip pointer către double în care este memorată adresa ultimului element al tabloului 𝑣 (double *p=&v[99];). Care dintre următoarele expresii afișează numărul de octeți pe care\nîi ocupă tabloul 𝑣 în memorie?",
      answers: [
    ...
    },
    {
      questionNumber: "41.",
      question:
        "Fie 𝑎 un tabloul bidimensional pătratic de dimensiune 𝑛. Care dintre următoarele secvențe de cod afișează elementele triunghiului delimitat de diagonala principală, ultima coloană și ultima linie din matricea 𝑎?",
      answers: [
        'for(int i=0;i<n;i++)\n  for(int j=0;j<n;j++)\n    if(i>=j) printf("%d ",a[i][j]);',
        'for(int i=0;i<n;i++)\n  for(int j=0;j<n;j++)\n    if(i<=j) printf("%d ",a[i][j]);',
        'for(int i=0;i<n;i++)\n  for(int j=0;j<=i;j++) printf("%d ",a[i][j]);',
